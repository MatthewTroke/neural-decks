...
func (jwtas *JWTAuthService) CreateAccessToken(name string, email string, userId string, image string, emailVerified bool, secret string) (string, error) {
	if secret == "" {
		return "", fmt.Errorf("secret is required to CreateAccessToken")
	}
...
func (jwtas *JWTAuthService) CreateRefreshToken(userId string, secret string) (string, error) {
	if secret == "" {
		return "", fmt.Errorf("secret is required to CreateRefreshToken")
	}
...
func (jwtas *JWTAuthService) RefreshAccessToken(refreshToken string, user *entities.User, secret string) (string, error) {
	if secret == "" {
		return "", fmt.Errorf("secret is required to RefreshAccessToken")
	}
	...
	newAccessToken, err := jwtas.CreateAccessToken(user.Name, user.Email, user.ID, user.Image, user.EmailVerified, secret)
	if err != nil {
...
func (jwtas *JWTAuthService) VerifyJWT(tokenString string, secret string) (*entities.CustomClaim, error) {

...
func RequireAuth(env *environment.Env) fiber.Handler {
	return func(c *fiber.Ctx) error {
		accessToken := c.Cookies("neural_decks_access_token")
		...
		token, err := jwt.ParseWithClaims(accessToken, &entities.CustomClaim{}, func(token *jwt.Token) (interface{}, error) {
		...
		// Extract claims
		claims, ok := token.Claims.(*entities.CustomClaim)

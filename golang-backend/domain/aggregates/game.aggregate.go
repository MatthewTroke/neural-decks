...
func (g *Game) WasAllPlayersCardCzar() bool {
	g.Lock()
	defer g.Unlock()
...
func (g *Game) PickNewCardCzar() error {
	...
	if g.WasAllPlayersCardCzar() {
		err := g.RemoveWasCardCzarFromAllPlayers()

		if err != nil {
			return fmt.Errorf("error removing card czar from all players: %w", err)
		}
	}
...
func (g *Game) ApplyEvent(event *events.GameEvent) error {
	g.Lock()
	defer g.Unlock()
	...
	case events.EventJoinedGame:
		...
		player, err := NewPlayer(payload.Claim, nil)
...
func (g *Game) BroadcastToAllPlayers(messageType string, payload interface{}) error {
	g.Lock()
	defer g.Unlock()

	if !g.HasPlayers() {
		return fmt.Errorf("could not broadcast to players, no players in game %s", g.ID)
	}

	var errors []string
	for _, player := range g.Players {
		if err := player.SendMessage(messageType, payload); err != nil {
			errors = append(errors, fmt.Sprintf("player %s: %v", player.UserID, err))
		}
	}

	if len(errors) > 0 {
		return fmt.Errorf("failed to broadcast to some players: %v", errors)
	}

	return nil
}

func (g *Game) BroadcastToAllPlayersExcept(messageType string, payload interface{}, exceptUserID string) error {
	g.Lock()
	defer g.Unlock()

...
type EventService struct {
	eventRepo        *repositories.RedisEventRepository
	gameStateService *GameStateService
}

func NewEventService(eventRepo *repositories.RedisEventRepository, gameStateService *GameStateService) *EventService {
	return &EventService{
		eventRepo:        eventRepo,
		gameStateService: gameStateService,
	}
...
func (s *EventService) AppendEvent(event events.GameEvent) error {
	return s.eventRepo.AppendEvent(event)
}

func (s *EventService) BuildGameByGameId(gameID string) (*aggregates.Game, error) {
...
func (s *EventService) rebuildGameFromEvents(initialGame *aggregates.Game) (*aggregates.Game, error) {
...
func (s *EventService) CreateGameEvent(gameID string, eventType events.GameEventType, payload interface{}) (events.GameEvent, error) {
	payloadJSON, err := json.Marshal(payload)

	if err != nil {
		return events.GameEvent{}, fmt.Errorf("failed to marshal payload: %w", err)
	}
...
func (s *EventService) GetEventsForGame(gameID string) ([]events.GameEvent, error) {
	return s.eventRepo.GetEventsForGame(gameID)
}

func (s *EventService) GetEventsSince(gameID string, since time.Time) ([]events.GameEvent, error) {
...
func (s *EventService) GetAllGamesWithCurrentState() ([]*aggregates.Game, error) {
	initialGames := s.gameStateService.GetAllGames()

	var currentGames []*aggregates.Game

	if len(initialGames) == 0 {
		return []*aggregates.Game{}, nil
	}

...
func (r *RedisEventRepository) AppendEvent(event events.GameEvent) error {
...
func (r *RedisEventRepository) GetEventsForGame(gameID string) ([]events.GameEvent, error) {
	ctx := context.Background()
...
func (r *RedisEventRepository) GetEventsSince(gameID string, since time.Time) ([]events.GameEvent, error) {
	allEvents, err := r.GetEventsForGame(gameID)
	if err != nil {
		return nil, err
	}

	var filteredEvents []events.GameEvent
...
func (r *RedisEventRepository) GetEventByID(eventID string) (*events.GameEvent, error) {
	ctx := context.Background()
